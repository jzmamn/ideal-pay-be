...
/** SP fixed fields — not component columns */
...
/**
 * Matrix keys are component NAMES (from _label columns) — used as pivot column headers.
 * The _labelToCode map resolves name → code for save.
 */
type AmountMatrix = Record<string, number[]>;
type HoursMatrix  = Record<string, number[]>;

/**
 * Flat row for NoPay tab — mapped directly from the SP result.
 * days / amount are mutable (edited in UI).
 */
export interface NopayFlatRow {
  empId:         number;
  employeeNo:    string;
  payrollName:   string;
  nopayRule:     string | null;  // nd_enp.name  — read-only
  nopayRuleDays: string | null;  // nd_emp.name  — read-only
  nopayCode:     string | null;  // nd_enp.code  — used in save payload
  days:          number;
  amount:        number;
}
...
/**
 * Normalise a PivotRow's keys to lowercase so the lookup is case-insensitive.
 * Spring's ColumnMapRowMapper may return keys in uppercase (e.g. NOPAY_RULE).
 */
...
/** Map flat SP nopay rows to NopayFlatRow[] — includes all active employees */
  ...
  return rows
    ...
    .map(raw => {
      ...
      return {
        ...
        nopayRuleDays: row['nopay_rule_days'] != null ? String(row['nopay_rule_days']) : null,
...
export class BatchComponent {
  ...
  /** Flat nopay rows — drives the NoPay tab table */
  ...
  save(): void {
    ...
    // ── Pivot-based sections (FA, FD, VA, VD, OT) ──────────────────────
    ...
    // TODO: replace 1 with current user ID from your AuthService
    const modifiedBy = 1;
  ...
  private _applyLoadResponse(resp: BatchLoadResponse): void {
    ...
    // ── Nopay flat rows ───────────────────────────────────────────────
    const nopayRows = resp.nopays ?? [];
    this._nopayRows.set(buildNopayFlatRows(nopayRows));
